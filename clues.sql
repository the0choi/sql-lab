...
-- There are only two cities she could be flying to in the country. One is named the same as the country – that would be too obvious. We're following our gut on this one; find out what other city in that country she might
-- be flying to.
...
WHERE c.name = 'San Marino' AND NOT ct.name = 'San Marino';
...
SELECT ct.name
FROM city ct
INNER JOIN country c ON c.capital = ct.id
WHERE c.name = 'Brazil';
